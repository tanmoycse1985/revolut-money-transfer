...
import com.moneytransfer.dao.H2DatabaseDAO;
...
public class AccountDAOImpl implements AccountDAO {
	...
	public List<Account> getAllAccounts() throws RevolutException {
		...
		try {
			...
			throw new RevolutException("getAccountById(): Error reading account data", e);
		} finally {
	 ...
	 * Get account by id
	 */
	public Account getAccountById(long accountId) throws RevolutException {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		Account acc = null;
	 ...
	 * Create account
	 */
	public long createAccount(Account account) throws RevolutException {
		Connection conn = null;
		PreparedStatement stmt = null;
		...
		try {
			...
			if (affectedRows == 0) {
				...
				throw new RevolutException("Account Cannot be created");
			}
			...
			throw new RevolutException("createAccount(): Error creating user account " + account, e);
		} finally {
	 ...
	 */
	public int deleteAccountById(long accountId) throws RevolutException {
		Connection conn = null;
		PreparedStatement stmt = null;
		try {
			conn = H2DatabaseDAO.getConnection();
			...
			throw new RevolutException("deleteAccountById(): Error deleting user account Id " + accountId, e);
		} finally {
	 ...
	 */
	public int updateAccountBalance(long accountId, BigDecimal deltaAmount) throws RevolutException {
		Connection conn = null;
		...
		try {
			...
			if (targetAccount == null) {
				throw new RevolutException("updateAccountBalance(): fail to lock account : " + accountId);
			}
			...
			if (balance.compareTo(MoneyUtil.zeroAmount) < 0) {
				throw new RevolutException("Not sufficient Fund for account: " + accountId);
			}
			...
			try {
				...
				throw new RevolutException("Fail to rollback transaction", re);
			}
		} finally {
			DbUtils.closeQuietly(conn);
	...
	public int transferAccountBalance(UserTransaction userTransaction) throws RevolutException {
		int result = -1;
		Connection conn = null;
		PreparedStatement lockStmt = null;
		PreparedStatement updateStmt = null;
		...
		try {
			...
			if (fromAccount == null || toAccount == null) {
				throw new RevolutException("Fail to lock both accounts for write");
			}
			...
			if (fromAccountLeftOver.compareTo(MoneyUtil.zeroAmount) < 0) {
				throw new RevolutException("Not enough Fund from source Account ");
			}
			// proceed with update
			updateStmt = conn.prepareStatement(SQL_UPDATE_ACC_BALANCE);
